...
def get_train_transforms_with_segmap():
    train_transforms = TrainTransformWithSegmap()
    return train_transforms
...
def get_test_transforms_with_segmap():
    test_transforms = TestTransformWithSegmap()
    return test_transforms


def get_object_transforms(mode):
    if mode == "test":
        pre_augmentations = []
        augmentations = []
    else:
...
def get_test_object_transforms():
    object_transforms = torch.nn.Sequential(
        OrderedDict(
            [
                ("pad_to_square", PadToSquare(fill=0, padding_mode="constant")),
